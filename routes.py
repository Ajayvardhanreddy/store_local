...
def dashboard():
    if session.get('username'):
        ...
        return render_template("dashboard.html", users_details=users_details, portfolio_details=portfolio_details)
    else:
        return 'login to view dashboard'
...
def portfolio(user_name):
    ...
    if portfolio_details:
        ...
        return render_template(templates_list[template],
                               ...
                               edu_len=edu_len, exp_len=exp_len,
...
def update_portfolio():
    portfolio_details = portfolio_details_col.find_one({'username': 'Ajayvardhan'})
    if portfolio_details:
        edu_len = len(portfolio_details['school_name'])
        exp_len = len(portfolio_details['company_name'])
        skills_len = len(portfolio_details['skills_list'])
...
@app.route('/insert')
def insert():
    users_accounts_col.insert_one({'name':'AA', 'age':0, 'location':'Ra'})
    return 'Done'
...
@app.route('/link')
def link():
    return render_template('portfolio_link.html', user_name='rits_ravula')
